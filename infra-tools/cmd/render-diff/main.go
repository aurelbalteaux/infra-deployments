...
import (
	...
	"github.com/redhat-appstudio/infra-deployments/infra-tools/internal/directpath"
...
func main() {
	...
	switch *detectionMode {
	case "appset":
		// Original mode: detect via ArgoCD ApplicationSets
		d, detErr := detector.NewDetector(headRef, baseRefRepo, *overlaysDir)
		if detErr != nil {
			logging.Fatal("initializing detector", "err", detErr)
		}
	...
	case "direct":
		// Direct mode: find kustomization directories directly
		d := directpath.NewDetector(headRef, *componentsDir, nil)
		affected, err = d.AffectedComponents(changedFiles)
		if err != nil {
			logging.Fatal("detecting affected components", "err", err)
		}
